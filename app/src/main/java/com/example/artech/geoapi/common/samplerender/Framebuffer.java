...
/*
  ...
  This code is an implementation of a Framebuffer class used for rendering to a texture.
  The class provides methods for resizing the framebuffer, getting the color and depth textures
  associated with it, and getting the width of the framebuffer.

  The constructor of the class creates the framebuffer object and binds it to the color and depth
  textures. The color and depth textures are created using the Texture class, which provides methods
  for specifying the format and parameters of the texture. The constructor also sets the dimensions of the framebuffer.

  The close() method is used to free the framebuffer and the textures when they are no longer needed.

  The resize() method is used to resize the framebuffer to the given dimensions. The method first checks
  if the dimensions have changed since the last call. If the dimensions are the same, the method returns
  without doing anything. If the dimensions have changed, the method updates the dimensions of the framebuffer
  and then updates the color and depth textures to match the new dimensions.

  The getColorTexture() and getDepthTexture() methods are used to retrieve the color and depth textures
  associated with the framebuffer, respectively.

  The getWidth() method is used to retrieve the width of the framebuffer.
